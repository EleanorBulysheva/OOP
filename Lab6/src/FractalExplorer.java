...
public class FractalExplorer {
    ...
    private class FractalWorker extends SwingWorker<Object, Object> {
        /*отвечает за вычисление одной строки фрактала */
        private int external_y;
        /*массив цветов данной строки*/
        private int[] RGBVals;

        //Передаём в конструкторе номер текущей строки с которой будем работать
        public FractalWorker(int y) {//принимает координаты каждого пикселя в одной строке
            external_y = y;
        }
        ...
        public Object doInBackground() {
            ...
            for (int x = 0; x < displaySize; x++) {
                ...
                /*сохранение каждого пикселя в массиве*/
                RGBVals[x] = rgbColor;
            }
            /*т.к. должен возвращать Object, возвращаем просто null, чтобы не выдавал ошибку*/
            return null;
        }
        // Метод, выполняемый после заверщения работы.
        public void done() {//обновляет дисплей с новой стракой
            /*перебор массива с пикселями которые были уже вычеслены в doInBackground */
            for (int x = 0; x < displaySize; x++) {
                image.drawPixel(x, external_y, RGBVals[x]);
            }
            /*чтобы не перерисовывать все изображение целиком укажем область*/
            image.repaint(0, 0, external_y, displaySize, 1);
            /*уменьшить количество оставшихся строк. если 0, то вывести enableUIThings*/
            rowsRemaining--;
            if (rowsRemaining < 1) {
                enableUIThings(true);
            }
    ...
    public void drawFractal() {
        enableUIThings(false);// вызов метода  enableUIThings
        rowsRemaining = displaySize;
        /*установление значения  rowsRemaining равным количеству
        строк которые нужно отрисовать*/
        for (int y = 0; y < displaySize; y++) {
            FractalWorker worker = new FractalWorker(y);
            worker.execute();/*в отображение создаем отдельный рабочий объект и
            метод execute, чтобы запустить основной поток и запустить задачу в фоновом режиме*/
        }
    }
    /**
